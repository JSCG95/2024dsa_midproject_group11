...
HYPOTHESIS: Lettuce fresh weight per plant is accurately estimated using canopy geometric features extracted using an RGB image analysis and machine learning models.
...
wr_data <-wr_data %>%
  ...
  mutate(trtname = as.factor(paste0(cultivar, '+',light))) %>%
...
```{r}
mean_lfw <- wr_data %>%
  group_by(fcultivar,dat,fdat,light,trtname,tray_id) %>%
  summarise(n = n(),
            mean = mean(lfw_g))
```
...
```{r}
Anova(mod1_df, type = 3)
```

```{r}
ACF(mod1_df, resType = "n") %>%
  plot(alpha = 0.01)
```
```{r}
anova(mod_default,mod_arma2, mod_AR1)
```
...
Here, I'm dropping the variables area_cm2 and diameter as I know ahead that incident light is capturing the same information being captured by these 2. 
As we will start using a machine learning approach to find the model that yields the most accurate prediction, our full dataset will be split in a training and a testing data set. Plant observation on the training set will be used to find the values of the hypertuning parameters for the models that require a hyperparameter specification (in our case: LASSO and random forest).  
