...
```{r}
ggplot(mod_gls_resid, aes(x=.fitted, y=.stdresid))+
...
response <- wr_data %>% dplyr::select(c('la_mm2'))

matrix <- predictors %>%
  mutate(la_mm2 = response$la_mm2) 

matrix_resp <- predictors %>%
  mutate(la_mm2 = wr_data$la_mm2,
         ldw_g = wr_data$ldw_g,
...
r2_mods <- matrix_resp %>%
  ...
  pivot_longer(cols = la_mm2:la_mm2,
               names_to = "resp_var",
               values_to = "resp_val"
               ) %>%
...
# Model fitting: Because SR is numerical, will give us a slope. 
lfw_quad<- lm(la_mm2 ~ inc_light + I(inc_light^2) ,
                 data = matrix_resp)
summary(lfw_quad)
...
ggplot(data = matrix_resp, aes(x = inc_light, y = la_mm2)) +
geom_point() +
geom_line(aes(y = fitted(lfw_quad)))
```

```{r}
lfw_log = nls(la_mm2 ~ SSlogis(inc_light, a, b,c), 
            data = matrix_resp)
...
```{r}
full_mlr<- lm(la_mm2 ~ ., data  = matrix)
summary(full_mlr)
...
```{r}
vif_mlr<- lm(la_mm2 ~ inc_light + convex_hull + compactness  , data  = matrix) 
...
index <- createDataPartition(matrix_alt$la_mm2, 
                             p=.8, 
                             list=FALSE, 
                             times=1)
...
performance_df <- map_dfr(names(predictions), ~{
  calculate_performance(predictions[[.x]], test_df$la_mm2) %>%
    mutate(names = .x) %>%
    arrange(desc(RMSE))
})
